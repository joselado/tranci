...
Basis_vector create(Basis_vector vin,int level) {
  ...
  if (nonex) {  // if create in a level which does not exist
    ...
    if (vin.basevec[level]) { // if occupied level, return zero
      ...
      vout.basevec[level] = true; //occupy level
      vout.sign = vin.sign*fermi_sign(vin,level); // multipli by the fermionic sign 
      return vout;
    }
  } // close nonzero input
}
