...
- `write.write_all(lowest, header, n, nbody)` assembles and writes `spectrum_ci.tex` only; the GUI is what invokes `pdflatex` (twice, for the table of contents). A library user calling `write_all` directly gets a `.tex` and no PDF. It populates `lowest.gs_manifold`/`lowest.manifolds` itself, so it no longer requires the caller to have run `get_gs_manifold()`/`disentangle_manifolds()` first. The preamble lives in `write.PREAMBLE` (pdflatex only: `lmodern`, `booktabs`, `longtable`, `hyperref`, `\ket`); tables go through `write.longtable`, numbers for tables through `numberformat.texnum` (never `fform`, which is the `.OUT` data-file format). The ground state plus the first `nbody` (default `write.nmanifolds_body`, 10) excited manifolds go in the body, the rest in an appendix; a non-degenerate ground state gets an expectation-value table instead of 1x1 matrices. `numberformat.recognise_number` only claims closed forms (`1/sqrt2`, `1/2`, ...) within `guess_tol` (1e-4).
